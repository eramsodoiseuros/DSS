...
CREATE TABLE IF NOT EXISTS `Armazem`.`Entrega` (
  ...
  `Palete_conteudo` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`codID`),
  INDEX `fk_Entrega_Palete_idx` (`Palete_codID` ASC, `Palete_conteudo` ASC) VISIBLE,
  CONSTRAINT `fk_Entrega_Palete`
    FOREIGN KEY (`Palete_codID` , `Palete_conteudo`)
    REFERENCES `Armazem`.`Palete` (`codID` , `conteudo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
